...
def import_cluster(api_endpoint, cloud_type, cluster_name, session):
    ...
    try:
        imported_cluster = session.post(api_endpoint + "/v1/spectroclusters/"
            + cloud_type + "/import", json=imported_cluster)
        imported_cluster.raise_for_status()
    except requests.exceptions.HTTPError as error:
        logger.error(error.response.text)
        sys.exit(1)
    imported_uid = imported_cluster.text.split("\"")[3]
    logger.info("imported a new cluster with uid " + imported_uid)
    return imported_uid
...
def get_manifest(api_endpoint, cluster_uid, session):
    ...
    try:
        imported_manifest = session.get(api_endpoint + "/v1/spectroclusters/"
            + cluster_uid + "/import/manifest")
    ...
    for cluster in clusters["items"]:
        if cluster["metadata"]["name"] == args.cluster_name[0]:
            ...
            # stop if we find a cluster with our name
                ...
                logger.info("will get manifest for existing cluster " +
                    cluster["metadata"]["name"] + " with id " + cluster["metadata"]["uid"])
                cluster_uid = cluster["metadata"]["uid"]
                break
    if cluster_uid is None:
        logger.info("no cluster found, doing import")
        cluster_uid = import_cluster(args.api_endpoint, args.cloud_type,
            args.cluster_name[0], session)
    return cluster_uid
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--api-endpoint", dest="api_endpoint", action="store",
        ...
        help="optionally configure the API endpoint location")

    # Project UID is a required parameter
    parser.add_argument("-u", "--project-uid", dest="project_uid", action="store",
        required=True, help="project UID - find in your Palette profile screen")
    ...
    cloud_types=['aws','azure','edge-native','edge','gcp','generic','libvirt'
        ,'maas','openstack','vsphere']
    parser.add_argument("-t", "--cloud-type", choices=cloud_types, action="store",
        required=True, help="set the cloud type to one of the supported options")
